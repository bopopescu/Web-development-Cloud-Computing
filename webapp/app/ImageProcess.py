...
def ImageTransSave(filePath,fileName):
    with Image(filename = os.path.join(filePath,fileName)) as img:
        with img.clone() as i:
            ...
            upload_to_s3(path_origin,config.s3_bucketname,fileName)
            os.remove(path_origin)
            path_origin = os.path.join(S3_ADDRESS,fileName)

        with img.clone() as i:
            i.resize(width = 150,height = 100)
            path_thumbnail = os.path.join(filePath,"thumbnail_"+fileName)
            i.save(filename=path_thumbnail)
            upload_to_s3(path_thumbnail,config.s3_bucketname,"thumbnail_"+fileName)
            os.remove(path_thumbnail)
            path_thumbnail = os.path.join(S3_ADDRESS,"thumbnail_"+fileName)

        with img.clone() as i:
            i.modulate(brightness=100, saturation=300, hue=100)
            path_a = os.path.join(filePath, "a_" + fileName)
            i.save(filename=path_a)
            upload_to_s3(path_a,config.s3_bucketname,"a_"+fileName)
            os.remove(path_a)
            path_a = os.path.join(S3_ADDRESS,"a_"+fileName)

        with img.clone() as i:
            i.modulate(brightness=100, saturation=100, hue=50)
            path_b = os.path.join(filePath, "b_" + fileName)
            i.save(filename=path_b)
            upload_to_s3(path_b,config.s3_bucketname,"b_"+fileName)
            os.remove(path_b)
            path_b = os.path.join(S3_ADDRESS,"b_"+fileName)

        with img.clone() as i:
            i.modulate(brightness=80, saturation=0, hue=100)
            path_c = os.path.join(filePath, "c_" + fileName)
            i.save(filename=path_c)
            upload_to_s3(path_c,config.s3_bucketname,"c_"+fileName)
            os.remove(path_c)
            path_c = os.path.join(S3_ADDRESS,"c_"+fileName)
