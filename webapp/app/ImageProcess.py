...
def ImageTransSave(username,filePath,fileName):
    with Image(filename = os.path.join(filePath,fileName)) as img:
        ...
        with img.clone() as i:
            ...
            path_thumbnail = os.path.join(S3_ADDRESS,'%s/%s' % (username, "thumbnail_"+fileName))
        ...
        with img.clone() as i:
            ...
            os.remove(path_a)
            path_a = os.path.join(S3_ADDRESS,'%s/%s' % (username, "a_"+fileName))
        ...
        with img.clone() as i:
            ...
            os.remove(path_b)
            path_b = os.path.join(S3_ADDRESS,'%s/%s' % (username, "b_"+fileName))
